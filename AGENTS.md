...
- Run `replay-env doctor` before trusting a new manifest or replay database.
- Prefer `replay-env run --trace-out ...` when a manifest declares commands or
  probes, so replay evidence is captured as a reusable trace.
